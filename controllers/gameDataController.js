...
async function updateGameData(req, res, id) {
  try {
    ...
    if (!game) {
      ...
      const { name, publisher, developer, description, price } = JSON.parse(
        body
      );
...
// @desc  Remove Game Data
// @route DELETE '/api/gamedata/:id'
async function deleteGameData(req, res, id) {
  try {
    const gameData = await GameData.findById(id);
